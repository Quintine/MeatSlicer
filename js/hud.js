...
function drawHUD(ctx) {
  const p = G.player;
  ctx.save();
  const panelH = p.holstered ? 140 : 112;
  ...
  // Hearts retain their illustrative icons on a cleaner fixed baseline.
  const hearts = Math.ceil(p.stats.maxHp / 2);
  const maxVitalIcons = 11;
  const shownHearts = Math.min(hearts, maxVitalIcons);
  for (let i = 0; i < shownHearts; i++) {
    const x = 21 + i * 22, y = 39;
    const fill = clamp(p.hp - i * 2, 0, 2) / 2;
    Sprites.draw(ctx, 'heart', x, y, 0, 18, false, 0.25);
    if (fill > 0) {
      ctx.save();
  ...
  const shownShields = Math.min(p.shieldHp, Math.max(0, maxVitalIcons - shownHearts));
  for (let i = 0; i < shownShields; i++) {
    const x = 21 + (shownHearts + i) * 22, y = 39;
  ...
  if (p.holstered) {
    ...
    ctx.fillStyle = 'rgba(86,40,48,0.35)'; ctx.fillRect(15, 112, 281, 20);
    Sprites.draw(ctx, 'w_' + p.holstered.id, 29, 122, 0, 28, false, 0.68);
    ctx.fillStyle = holsterEmpty ? '#d94652' : '#aa9290'; ctx.font = 'bold 9px monospace'; ctx.textAlign = 'left';
    ctx.fillText((holsterEmpty ? '[EMPTY] ' : '[R] ') + WEAPONS[p.holstered.id].name.toUpperCase(), 47, 125);
    ctx.fillStyle = holsterEmpty ? '#ef4057' : '#d0aa4a'; ctx.textAlign = 'right';
    ctx.fillText(holsterEmpty ? 'NEEDS AMMO' : Math.ceil(p.holstered.ammo) + ' AMMO', 289, 125);
  }
  ...
  if (itemEntries.length) {
    ...
    for (let index = 0; index < itemEntries.length; index++) {
      ...
      Sprites.draw(ctx, 'i_' + iid, ix, iy, 0, 30, false, 0.95);
      if (lvl > 1) {
  ...
  ctx.textAlign = 'left'; ctx.fillStyle = G.pressure > 1 ? '#d6aa39' : '#69b8bd'; ctx.font = 'bold 8px monospace';
  ctx.fillText('PRESSURE ' + Math.round(G.pressure * 100) + '%', W - 168, H - 28);
